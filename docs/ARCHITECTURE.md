...
| 7 | **The gate stops being trustworthy.** Green on both platforms is what a milestone means (SPEC §3 rule 8), so a gate that is slow or flaky gets waved through. | **Open, and it has already happened.** Three of the four Working Copy tests hung indefinitely on one Linux run and nothing stopped them: no job carried a timeout, so the run sat at fifty minutes heading for the six-hour default. Two causes were separable and are fixed — no timeouts, and a cache that outgrew GitHub's 10 GB allowance and evicted itself, which is what a 28-minute cold run was. The hang itself is not understood: twelve local runs at the same parallelism never reproduced it, and CI now runs the tests serially, which is a mitigation. The next Linux hang is the evidence to chase; until then a GPUI test driven against a real repository is not known to be safe in parallel. |
